 ...
 * 
 * @author tity 2017-12-15
 * @Url: http://www.jhmis.com
 * @version 1.0v
 */
(function() {
    ...
    jh = {
	   ...
	   /**使用jh.open代替top.layer.open，参数使用完全一致，请参照layer官网, 不在直接暴露layer,对layer进行统一封装**/
	   open:top.layer.open,
	   openParent:parent.layer.open,
	   ...
	   //layer之外的另一个选择toast风格消息提示框,直接使用jh.toastr调用
	   toastr:(function(){
		   top.toastr.options = {
					  "closeButton": true,
					  "debug": false,
    ...
    //打开对话框(添加修改)
	  openDialog: function(title,url,width,height, $table){
        ...
        openDialogMap: function(title,url,width,height, $table){
            var auto = true;//是否使用响应式，使用百分比时，应设置为false
            if(width.indexOf("%")>=0 || height.indexOf("%")>=0 ){
                auto =false;
            }
            var aim = top.layer.open({
                type: 2,
                area: [width, height],
                title: title,
                auto:auto,
                ...
                yes: function(index, layero){
                    var iframeWin = layero.find('iframe')[0]; //得到iframe页的窗口对象，执行iframe页的方法：iframeWin.method();
                    var position = layero.find("iframe")[0].contentWindow.$("#l-position").text();
                    if(!position){
                        alert("请选择地点");
                    	return;
					}
        ...
        //打开对话框(通过回调传值,回调根据不同页面自己定义)
        openDialogPass: function(title,url,width,height, fn){
            var auto = true;//是否使用响应式，使用百分比时，应设置为false
            if(width.indexOf("%")>=0 || height.indexOf("%")>=0 ){
                auto =false;
            }
            var ltop = top.layer.open({
                type: 2,
                area: [width, height],
                title: title,
                auto:auto,
                ...
                btn: ['确定', '关闭'],
                yes: function(index, layero){
                    var obj = fn(index, layero);
                    if(obj.flag){
                    	top.layer.close(ltop);
					}else{
                        top.layer.alert(obj.msg,{icon:2});
					}
                },
                cancel: function(index){
                }
