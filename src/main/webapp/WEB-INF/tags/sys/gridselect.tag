...
<%@ attribute name="changeFunc" type="java.lang.String" required="false" description="指定change事件函数"%>
        ...
        if('${id}'=="openStoreGoods"){
            top.layer.open({
                ...
                yes: function(index, layero){
                    ...
                    $(".categoryPid").val(item.categoryPid);
                    $(".categoryName").val(item.categoryPname);
                    $(".goodsCode").val(item.code);
                    $(".goodsName").val(item.goodsName)
                    top.layer.close(index);//关闭对话框。
                    if("${changeFunc}" && (oldValue != newValue)){
                        ${changeFunc}(newValue);
                    }
                },
            ...
            top.layer.open({
                ...
                yes: function(index, layero){
                    ...
                    for(var i=0; i<items.length; i++){
                        var item = items[i];
                        ids.push(item.id);
                        names.push(item.${fn:substring(labelName, fns:lastIndexOf(labelName, '.')+1, fn:length(labelName))})
                    }
                    var oldValue = $("#${id}Id").val();
