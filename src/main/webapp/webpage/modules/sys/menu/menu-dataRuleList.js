...
$(document).ready(function() {
	$('#dataRuleTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/sys/dataRule/form?id=" + row.id;
                        ...
                        jh.confirm('确认要删除该数据权限记录吗？', function(){
                       	jh.loading();
                       	jh.get("${ctx}/sys/dataRule/delete?id="+row.id, function(data){
                   	  		if(data.success){
			...
			,{
		        ...
		        sortable: true,
		        formatter:function(value, row , index){
		        	return jh.getDictLabel(${fns:toJson(fns:getDictList('t_express'))}, value, "-");
		        }
			...
			,{
                ...
                events: {
    		        ...
    		        'click .edit': function (e, value, row, index) {
    		        	...
    		        	jh.openDialog('编辑数据规则', '${ctx}/sys/dataRule/form?id='+row.id+'&menuId='+$("#menuId").val(),'800px', '580px', $("#dataRuleTable"));
    		        }
    		    },
	  ...
	  $('#dataRuleTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
		...
		$("#dataRuleAddButton").click(function(){
			...
			jh.openDialog('添加数据规则', '${ctx}/sys/dataRule/form?menuId='+$("#menuId").val(),'800px', '580px', $("#dataRuleTable"));
  ...
  function deleteAll(){
		...
		jh.confirm('确认要删除该数据权限记录吗？', function(){
			jh.loading();  	
			jh.get("${ctx}/sys/dataRule/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#dataRuleTable').bootstrapTable('refresh');
         	  			jh.success(data.msg);
         	  		}else{
