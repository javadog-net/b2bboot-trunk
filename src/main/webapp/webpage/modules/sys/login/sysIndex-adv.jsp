...
<head>
	...
	<link href="${ctxStatic}/common/css/adv-menu.css" rel="stylesheet" />
...
<body class="${cookie.white.value}  adv-menu">
<input id="updatepassword" type="hidden" value="${updatepassword }" />
<sys:message content="${updatepassword}" showType="2" type="warning"/>
<nav class="navbar navbar-inverse display-default opnav-navbar navbar-fixed-top" role="navigation">
	<div class="topnav-navbar">
		<div class="navbar-header text-center">
			...
			<a class="navbar-brand J_menuItem" href="${ctx}/home" data-index="0">${fns:getConfig('productName')}</a>
		</div>
	</div>
	<div class="navbar-container container-fluid">
		<div class="collapse navbar-collapse navbar-collapse-toolbar row">
            <div class="pull-left" style="width: calc( 100% - 680px)" id="advMenu">
                <ul id="advMenu_ul" class="nav navbar-toolbar nav-tabs navbar-left" role="tablist">
                    <t:advMenu  menu="${fns:getTopMenu()}" position="top"></t:advMenu>
                </ul>
            </div>
            <button id="drop_advMenu" title="更多菜单">
							...
							<section id="col-left" class="nano">
								<div id="col-left-inner" class="nano-content">
									...
									<div class="collapse navbar-collapse navbar-ex1-collapse tab-content" id="sidebar-nav">
										...
										<t:advMenu  menu="${fns:getTopMenu()}" position="left"></t:advMenu>
									</div>
								</div>
							</section>
						</div>
...
<section id="body-container" class="animation">
	...
	<div class="footer">
		<div class="pull-left">${fns:getConfig('productName')} &copy; 2017-2025</div>
	</div>
...
<script>
     ...
     * 处理更多菜单显示
     */
    function processDropAdvMenu(){
        //计算顶部导航高度是否超过51px
        var advMenuHeight = $('#advMenu_ul').height();
        if(advMenuHeight > 51){
            $('#drop_advMenu').show();
        } else {
            $('#drop_advMenu').hide();
    ...
    $(function ($) {
        processDropAdvMenu();
        //显示所有顶部菜单点击事件
        $('#drop_advMenu').on('click', function (e) {
            var advMenu = $('#advMenu');
            advMenu.toggleClass('all');
        });
        //顶部导航菜单离开事件
        $('#advMenu_ul').on('mouseleave', function (e) {
            var advMenu = $('#advMenu');
            if (advMenu.hasClass('all')) {
                advMenu.removeClass('all');
            }
        });
	...
	　
   　　var myName="<%=session.getAttribute("updatepassword")%>";
　console.log("222323"+myName);
				...
				yes: function(index, layero) {
					...
					if (inputForm.valid()) {
						if (inputForm.find("#newPassword").val() != inputForm.find("#confirmNewPassword").val()) {
							jh.alert("输入的2次新密码不一致，请重新输入！")
							return;
						}
						jh.post("${ctx}/sys/user/savePwd?oldPassword=" + inputForm.find("#oldPassword").val() + "&newPassword=" + inputForm.find("#newPassword").val(), $('#inputForm').serialize(), function(data) {
							if (data.success) {
								jh.success(data.msg);
								jh.close(index); //关闭dialog
								window.location.reload();
