...
<script>
		$(document).ready(function() {
					  ...
					  //请求方法
		                ...
		                columns: [{
					        ...
					        formatter:function(value, row, index){
					        	return '<a  href="#" onclick="jh.openDialogView(\'查看角色\', \'${ctx}/sys/role/form?id='+row.id+'\',\'800px\', \'500px\')">'+value+'</a>'
					        }
	                        ...
	                        events: {
	            		        'click .view': function (e, value, row, index) {
	            		        	jh.openDialogView('查看角色', '${ctx}/sys/role/form?id=' + row.id,'800px', '500px');
	            		        },
	            		        'click .edit': function (e, value, row, index) {
	            		        	jh.openDialog('编辑角色', '${ctx}/sys/role/form?id=' + row.id,'800px', '500px');
	            		        },
	            		        'click .del': function (e, value, row, index) {
	            		        	del(row.id);
	            		        },
	            		        'click .auth': function (e, value, row, index) {
	            		        	jh.openDialog('菜单权限', '${ctx}/sys/role/auth?id=' + row.id,'350px', '700px');
	            		        },
		  ...
		  function add(){
			  jh.openDialog('新建角色', '${ctx}/sys/role/form','800px', '500px')
		  }
		  ...
		  function edit(id){
			  ...
			  jh.openDialog('编辑角色', "${ctx}/sys/role/form?id=" + id,'800px', '500px')
		  ...
		  function del(ids){
				...
				jh.confirm('确认要删除选中角色吗？',  function(){
					jh.loading();
             	  	jh.get("${ctx}/sys/role/delete?ids=" + ids, function(data){
             	  		if(data.success){
             	  			$('#table').bootstrapTable('refresh');
            	  			jh.success(data.msg);
            	  		}else{
