...
<head>
	...
	function modifyMail(){
		...
		jh.post("${ctx}/sys/systemConfig/save",{"smtp":$("#smtp").val(),"port":$("#port").val(),"mailName":$("#mailName").val(),"mailPassword":$("#mailPassword").val()}, function (data) {
	    	    if(data.success){
	...
	function modifySms(){
		jh.post("${ctx}/sys/systemConfig/save",{"smsName":$("#smsName").val(),"smsPassword":$("#smsPassword").val()},function (data) {
	    	    if(data.success){
	    	    	top.layer.alert("更新成功！");
	    	    }else{
	    	    	top.layer.alert("更新失败！");
