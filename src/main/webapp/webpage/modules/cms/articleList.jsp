...
<div class="panel panel-primary">
	...
	<form:form id="searchForm" modelAttribute="article" action="${ctx}/cms/article/" method="post" class="form form-horizontal well clearfix">
		...
		<sys:treeselect id="category" name="category.id" value="${article.category.id}" labelName="category.name" labelValue="${article.category.name}"
					title="栏目" url="/cms/category/treeData" module="article" allowClear="true" notAllowSelectRoot="false" cssClass="form-control"/>	
		</div>
		<div class="col-xs-4 col-sm-3 col-md-3">
		...
		<label>状态：</label>
		<form:radiobuttons onclick="$('#searchForm').submit();" path="delFlag" items="${fns:getDictList('cms_del_flag')}" itemLabel="label" itemValue="value" htmlEscape="false" class="form-control i-checks" />
		</div>
		</div>
		<div class="col-xs-4 col-sm-3 col-md-3">
		<div style="margin-top:26px">
