...
$(document).ready(function() {
	$('#sysDataSourceTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	edit(row.id);
                        ...
                        jh.confirm('确认要删除该数据源记录吗？', function(){
                       	jh.loading();
                       	jh.get("${ctx}/tools/sysDataSource/delete?id="+row.id, function(data){
                   	  		if(data.success){
			   ...
			   ,{
				   ...
				   sortable: true,
				   formatter:function(value, row , index){
					   return jh.getDictLabel(${fns:toJson(fns:getDictList('db_driver'))}, value, "-");
				   }
  ...
  function deleteAll(){
		...
		jh.confirm('确认要删除该数据源记录吗？', function(){
			jh.loading();  	
			jh.get("${ctx}/tools/sysDataSource/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#sysDataSourceTable').bootstrapTable('refresh');
         	  			jh.success(data.msg);
         	  		}else{
   ...
   function add(){
	  jh.openDialog('新增数据源', "${ctx}/tools/sysDataSource/form",'800px', '500px', $('#sysDataSourceTable'));
  }
  ...
  <shiro:hasPermission name="tools:sysDataSource:edit">
	  jh.openDialog('编辑数据源', "${ctx}/tools/sysDataSource/form?id=" + id,'800px', '500px', $('#sysDataSourceTable'));
  </shiro:hasPermission>
   <shiro:lacksPermission name="tools:sysDataSource:edit">
	  jh.openDialogView('查看数据源', "${ctx}/tools/sysDataSource/form?id=" + id,'800px', '500px', $('#sysDataSourceTable'));
