...
$(document).ready(function() {
	$('#storeJoininTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/shop/storeJoinin/form?id=" + row.id;
                   } else if($el.data("item") == "delete"){
                        jh.confirm('确认要删除该店铺申请记录吗？', function(){
                       	jh.loading();
                       	jh.get("${ctx}/shop/storeJoinin/delete?id="+row.id, function(data){
                   	  		if(data.success){
                   	  			$('#storeJoininTable').bootstrapTable('refresh');
                   	  			jh.success(data.msg);
                   	  		}else{
                   	  			jh.error(data.msg);
                   	  		}
			...
			,{
		        ...
		        ,formatter:function(value, row , index){
		        	return "<a href='${ctx}/shop/storeJoinin/form?id="+row.id+"'>"+value+"</a>";
		         }
	  ...
	  if(navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)){//如果是移动端
		  ...
		  $('#storeJoininTable').bootstrapTable("toggleView");
		}
	  
	  $('#storeJoininTable').on('check.bs.table uncheck.bs.table load-success.bs.table ' +
                'check-all.bs.table uncheck-all.bs.table', function () {
          var sels = $('#storeJoininTable').bootstrapTable('getSelections');
			...
			jh.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/shop/storeJoinin/import/template';
				  },
  ...
  function getIdSelections() {
        return $.map($("#storeJoininTable").bootstrapTable('getSelections'), function (row) {
            return row.id
        });
    }
  ...
  function deleteAll(){
		...
		jh.confirm('确认要删除该店铺申请记录吗？', function(){
			jh.loading();  	
			jh.get("${ctx}/shop/storeJoinin/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#storeJoininTable').bootstrapTable('refresh');
         	  			jh.success(data.msg);
         	  		}else{
  ...
  function edit(){
	  window.location = "${ctx}/shop/storeJoinin/form?id=" + getIdSelections();
  }
...
function audit(id){
    ...
    jh.open({
        ...
        btn1: function(index, layero){
            ...
            jh.loading('  正在审核，请稍等...');
            jh.post("${ctx}/shop/storeJoinin/audit",inputForm.serialize(),function(data){
                if(data.success){
                    $('#storeJoininTable').bootstrapTable('refresh');
                    jh.success(data.msg);
                }else{
