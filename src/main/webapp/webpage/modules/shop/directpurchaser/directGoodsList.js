...
$(document).ready(function() {
			...
			,{
			 ...
			 , {
                 ...
                 formatter:  function operateFormatter(value, row, index) {
                     return [
                    	 "<a href='javascript:savePurchaserGoodsRel(\""+row.code+"\")'>上架</a>"
                 ].join('');
                 }
             }
		     ]
  ...
  function savePurchaserGoodsRelAll(){

		jh.confirm('确认要上架该商品吗？', function(){
			jh.loading();
			jh.get("${ctx}/purchasergoodsrel/purchaserGoodsRel/savePurchaserGoodsRel?ids=" + getIdSelections()+"&purchaserId=${pid}", function(data){
         	  		if(data.success){
         	  			$('#goodsTable').bootstrapTable('refresh');
         	  			jh.success(data.msg);
         	  		}else{
  ...
  function savePurchaserGoodsRel(id){

		jh.confirm('确认要上架该商品吗？', function(){
			jh.loading();
			var purchaserId=$('#purchaserId').val();
			jh.get("${ctx}/purchasergoodsrel/purchaserGoodsRel/savePurchaserGoodsRel?ids=" +id+"&purchaserId=${pid}", function(data){
       	  		if(data.success){
       	  			$('#goodsTable').bootstrapTable('refresh');
       	  			jh.success(data.msg);
       	  		}else{
  ...
  function add(aim){
      ...
      window.location.href="${ctx}/shop/directgoods/form?categoryId=" + catid + "&categoryPid=" + catparid;
  }
  $(function(){
      //传递分类id
