        ...
        $(document).ready(function() {
            validateForm = $("#inputForm").validate({
                submitHandler: function(form){
                    jh.post("${ctx}/shop/directpurchaser/directpurchaser/saveAccount",$('#inputForm').serialize(),function(data){
                        if(data.success){
...
<body class="bg-white">
<form:form id="inputForm" modelAttribute="directPurchaserAccount" class="form-horizontal">
    <form:hidden path="id"/>
    ...
    <table class="table table-bordered">
        ...
        <tr>
            ...
            <td class="width-35">
                <form:input path="loginName" htmlEscape="false"    class="form-control required" readonly="${empty purchaserAccount.id?false:true}" />
            </td>
        ...
        <tr>
            <td class="width-15 active"><label class="pull-right"><c:if test="${empty purchaserAccount.id}"><font color="red">*</font></c:if>登录密码：</label></td>
            <td class="width-35">
                <form:input path="newPassword" htmlEscape="false"    class="form-control ${empty purchaserAccount.id?'required':''}"/>
                <c:if test="${not empty purchaserAccount.id}"><span class="help-inline">若不修改密码，请留空。</span></c:if>
            </td>
            <td class="width-15 active"><label class="pull-right"><c:if test="${empty purchaserAccount.id}"><font color="red">*</font></c:if>确认密码：</label></td>
            <td class="width-35">
                <input id="repass" class="form-control ${empty purchaserAccount.id?'required':''}"/>
            </td>
        </tr>
        <tr>
