...
$(document).ready(function() {
	$('#directPurchaserTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/shop/directpurchaser/directpurchaser/subreldata?id=${pid}",
...
//           }
           , {
                       ...
                       formatter:  function operateFormatter(value, row, index) {
                           return [
                        	   "<a href='javascript:deletePurchaserRel(\""+row.id+"\")'>取消关联</a>"
                           ].join('');
                       }
                   }
		     ]
  ...
  function deletePurchaserRelAll(){

		jh.confirm('确认移除子账号吗？', function(){
			jh.loading();  	
			jh.get("${ctx}/purchasermainrel/purchaserMasterSlaveRel/deletePurchaserRelAll?ids=" + getIdSelections()+"&purchaserId=${pid}", function(data){
         	  		if(data.success){
         	  			$('#directPurchaserTable').bootstrapTable('refresh');
         	  			jh.success(data.msg);
         	  		}else{
  ...
  function deletePurchaserRel(id){

		jh.confirm('确认移除子账号吗？', function(){
			jh.loading();  	
			jh.get("${ctx}/purchasermainrel/purchaserMasterSlaveRel/deletePurchaserRelAll?ids=" +id+"&purchaserId=${pid}", function(data){
       	  		if(data.success){
       	  			$('#directPurchaserTable').bootstrapTable('refresh');
       	  			jh.success(data.msg);
       	  		}else{
  ...
  function  toSetPurchaserRel(){
	  window.location.href = "${ctx}/shop/directpurchaser/directpurchaser/toNoRelList?pid=${pid}";
  }
