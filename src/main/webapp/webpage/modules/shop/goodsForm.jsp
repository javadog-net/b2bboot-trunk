            ...
            <div class="panel panel-primary">
                <div class="panel-heading">
                    <h3 class="panel-title">
                        <a class="panelButton" href="${ctx}/shop/goods?repage"><i class="ti-angle-left"></i> 返回</a>
                    </h3>
                </div>
                    ...
                    <form:form id="inputForm" modelAttribute="goods" action="${ctx}/shop/goods/save" method="post"
                        ...
                        <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <form:hidden id="mainPicUrl" path="mainPicUrl" htmlEscape="false"  class="form-control"/>
                                <sys:webupload input="mainPicUrl" type="file"  multiple="true"/>
...
event.keyCode<48 || event.keyCode>57)) event.returnValue=false"/>
                            ...
                            <div class="form-group">
                             <label class="col-sm-2 control-label">商品模版：</label>
                                <div class="col-sm-10">
                                 <form:select path="goodsTemplate" value="${goods.goodsTemplate}" htmlEscape="false"
                                                 class="form-control">
                                        <form:option value="" label="请选择商品模版" selected="true"/>
                                        <form:options items="${goodsTemplate}" htmlEscape="false"/>
                                    </form:select>
                                </div>
        ...
        $.ajax({
            ...
            success: function (data) {
               ...
               if(data.success){
                   ...
                   var imgArr = imgList.split("|");
                    if(resImg){
                        ...
                        for(var i=0;i<resImg.length; i++){
                            ...
                            if(imgArr.indexOf(resImg[i].url) ==-1){
                                imgArr.push(resImg[i].url);
                            }
                            var file = newWebUploadFile(resImg[i].url);
                            wb.uploader.onFileQueued(file);
                        }
                        $(".imgshow").append(str);
                   ...
                   var endStr = imgArr.join("|");
                    if(endStr.substring(endStr.length-1,endStr)=="|"){
                        endStr = endStr.substring(0,endStr.length-1);
                    }
                    $("#mainPicUrl").val(endStr);
               }else{
