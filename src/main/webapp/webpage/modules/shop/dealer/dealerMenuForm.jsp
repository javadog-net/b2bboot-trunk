		...
		$(document).ready(function() {
			validateForm = $("#inputForm").validate({
				submitHandler: function(form){
						jh.post("${ctx}/shop/dealer/dealerMenu/save",$('#inputForm').serialize(),function(data){
		                    if(data.success){
		                    	var current_id = data.body.dealerMenu.id;
		                    	var target = $dealerMenuTreeTable.get(current_id);
		                    	var old_parent_id = target.attr("pid") == undefined?'1':target.attr("pid");
		                    	var current_parent_id = data.body.dealerMenu.parentId;
		                    	var current_parent_ids = data.body.dealerMenu.parentIds;
		                    	
		                    	if(old_parent_id == current_parent_id){
		                    		if(current_parent_id == '1'){
		                    			$dealerMenuTreeTable.refreshPoint(-1);
		                    		}else{
		                    			$dealerMenuTreeTable.refreshPoint(current_parent_id);
		                    		}
		                    	}else{
		                    		$dealerMenuTreeTable.del(current_id);//刷新删除旧节点
		                    		$dealerMenuTreeTable.initParents(current_parent_ids, "1");
		                    	}
...
<body class="bg-white">
		<form:form id="inputForm" modelAttribute="dealerMenu" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<table class="table table-bordered">
		   <tbody>
				<tr>
					...
					<td class="width-35">
						<sys:treeselect id="parent" name="parent.id" value="${dealerMenu.parent.id}" labelName="parent.name" labelValue="${dealerMenu.parent.name}"
						title="上级菜单" url="/shop/dealer/dealerMenu/treeData" extId="${dealerMenu.id}" cssClass="form-control required" allowClear="true"/>
					</td>
				...
				<tr>
					...
					<td class="width-35">
						<sys:layericonselect id="icon" name="icon" value="${dealerMenu.icon}"/>
					</td>
