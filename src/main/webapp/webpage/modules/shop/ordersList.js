...
$(document).ready(function() {
		  ...
		  //请求方法
               ...
               onContextMenuItem: function(row, $el){
                   if($el.data("item") == "edit"){
                   	window.location = "${ctx}/shop/orders/form?id=" + row.id;
                   } else if($el.data("item") == "delete"){
                        jh.confirm('确认要删除该订单记录吗？', function(){
                       	jh.loading();
                       	jh.get("${ctx}/shop/orders/delete?id="+row.id, function(data){
                   	  		if(data.success){
		   ...
		   ,{
			...
			,{
		        ...
		        sortable: true,
		        formatter:function(value, row , index){
		        	return jh.getDictLabel(${fns:toJson(fns:getDictList('evaluation_state'))}, value, "-");
		        }
			...
			,{
		        ...
		        sortable: true,
		        formatter:function(value, row , index){
		        	return jh.getDictLabel(${fns:toJson(fns:getDictList('order_state'))}, value, "-");
		        }
		   ...
		   ,{
			 ...
			 *
			 ,{
		        ...
		        sortable: true,
		        formatter:function(value, row , index){
		        	return jh.getDictLabel(${fns:toJson(fns:getDictList('payment_kind'))}, value, "-");
		        }
			...
			,{
		        ...
		        sortable: true,
		        formatter:function(value, row , index){
		        	return jh.getDictLabel(${fns:toJson(fns:getDictList('order_refund_state'))}, value, "-");
		        }
			...
			jh.open({
			    ...
			    btn: ['下载模板','确定', '关闭'],
				    btn1: function(index, layero){
					  window.location='${ctx}/shop/orders/import/template';
				  },
  ...
  function deleteAll(){
		...
		jh.confirm('确认要删除该订单记录吗？', function(){
			jh.loading();  	
			jh.get("${ctx}/shop/orders/deleteAll?ids=" + getIdSelections(), function(data){
         	  		if(data.success){
         	  			$('#ordersTable').bootstrapTable('refresh');
         	  			jh.success(data.msg);
         	  		}else{
