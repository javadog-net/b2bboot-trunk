	...
	<div class="panel panel-primary">
		...
		<div class="panel-body">
			...
			<div class="">
                ...
                <table class="table table-striped table-hover table-bordered" style="margin-top: 20px">
                    ...
                    <tr class="active">
                        ...
                        <td>
                            ...
                            <c:if test="${shopMsgCustcodeOrder.cancelFlag=='1'}">跟进中</c:if>
				...
				<table class="table table-striped table-hover table-bordered" style="margin-top: 20px">
					<h3 style="font-size: 17px; font-weight: bold; margin-bottom: 10px;">零售订单相关信息</h3>
					<tbody>
					<tr class="active">
						<td>需求公司名:</td>
						<td>
							${shopMsgZykc.companyName}
						</td>
						<td>经销商编码:</td>
						<td>
							${shopMsgZykc.custcode}
						</td>
					</tr>
					<tr >
						<td>添加时间:</td>
						<td>
							<fmt:formatDate value="${shopMsgZykc.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
						</td>
						<td>交易金额:</td>
						<td>
							${shopMsgZykc.tradeCount}
						</td>
					</tr>

					<c:if test="${not empty shopMsgZykc.cancelType}">
					<tr class="active">
						<td>废弃类型:</td>
						<td>
							<c:if test="${shopMsgZykc.cancelType=='0'}">未处理</c:if>
							<c:if test="${shopMsgZykc.cancelType=='1'}">跟进中</c:if>
						</td>
						<td>废弃理由:</td>
						<td>
								${shopMsgZykc.cancelReson}
						</td>
					</tr>
					<tr >
						<td>废弃描述:</td>
						<td>
							${shopMsgZykc.cancelDesc}
						</td>
						<td>废弃时间:</td>
						<td>
							<fmt:formatDate value="${shopMsgZykc.cancleDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
						</td>
					</tr>
					</c:if>
					<tr class="active">
						<td>审核状态:</td>
						<td>
							<c:if test="${shopMsgZykc.isCheck==''}">待审核</c:if>
							<c:if test="${shopMsgZykc.isCheck=='0'}">通过</c:if>
							<c:if test="${shopMsgZykc.isCheck=='1'}">拒绝</c:if>
						</td>
						<td>是否关闭:</td>
						<td>
							<c:if test="${shopMsgZykc.isClosed=='0'}">否</c:if>
							<c:if test="${shopMsgZykc.isClosed=='1'}">是</c:if>
						</td>
					</tr>
					<c:if test="${not empty shopMsgZykc.isCheck}">
						<tr >
							<td>审核人编号:</td>
							<td>
								${shopMsgZykc.checker}
							...
							<td>审核时间:</td>
							<td>
								<fmt:formatDate value="${shopMsgZykc.checkDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
							</td>
						</tr>
					</c:if>
					<tr class="active">
						<td>见证性材料:</td>
						<td>
							<c:forEach var="url" items="${shopMsgZykc.imageUrl.split(',')}">
								<img src="${url}" alt="" style="width: 200px;height: 200px; float: left; margin-left: 10px"/>
							</c:forEach>
						</td>
					</tr>
					</tbody>
