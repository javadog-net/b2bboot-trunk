...
$(document).ready(function() {
	$('#shopMsgTable').bootstrapTable({
		  ...
		  //请求方法
               ...
               url: "${ctx}/process/shopmsg/shopMsg/data",
			...
			,{
		        ...
		        ,formatter:function(value, row , index){
		        	return "<a style='color: #3ca2e0!important;text-decoration: none!important;' href='${ctx}/process/shopmsg/shopMsg/view?id="+row.id+"'>"+value+"</a>";
		         }
			   ...
			   sortable: true
		   ...
		   },
				   {
					   ...
					   formatter:  function operateFormatter(value, row, index) {
					   	   var retStr = "<a class='btn btn-success' href='${ctx}/process/shopmsg/shopMsg/view?id="+row.id+"'>详情</a> ";
						   if(row.msgFlag=="0"){
							<shiro:hasPermission name="process:shopmsg:shopMsg:ptcheck">
							   retStr = retStr.concat('<a href="#" class="check btn btn-default"  title="审核">审核</a> ');
						   </shiro:hasPermission>
						   }
						   if(row.isDispatch=="0" && row.msgFlag=="0"){
						   	    var org = "0";
						   	  	if(row.companyOrgCode){
									org = "1";
								}
						   <shiro:hasPermission name="process:shopmsg:shopMsg:dispatcher">
							   retStr = retStr.concat("<a class='btn btn-primary' onclick='dispatch(\""+ row.id +"\",\""+ org +"\")'>派单</a> ");
						   </shiro:hasPermission>
						   }
						   if(row.msgFlag=="60"){
						   <shiro:hasPermission name="process:shopmsg:shopMsg:restart">
							   retStr = retStr.concat("<a class='btn btn-warning restart'>开启</a> ");
						   </shiro:hasPermission>
						   }
						   return retStr;
					   }
  ...
  function dispatch(id,companyOrgCode){
	...
	function exportData(){
  	...
  	window.location.href = "${ctx}/process/shopmsg/shopMsg/export?" + formData;
  }
</script>