...
<head>
	...
	<script type="text/javascript">
        ...
        function refreshGroups() {
            jh.get("${ctx}/iim/layGroup/data", function (data) {
        ...
        function addGroup(){
            jh.openDialog("创建群组","${ctx}/iim/layGroup/form","800px", "500px", refreshGroups);

        }
        function delGroup(id) {
            jh.confirm("确认解散该群吗？",function () {
                jh.get('${ctx}/iim/layGroup/delete?id='+id, function (data) {
                    if(data.success){
                        jh.success("解散成功!");
                        refreshGroups();
                    }
        ...
        function editGroup(id) {
            jh.openDialog("创建群组","${ctx}/iim/layGroup/form?id="+id,"800px", "500px", refreshGroups);

        }
        function viewGroupMember(id) {
           jh.openDialogView("查看群成员","${ctx}/iim/layGroup/member-view?id="+id,"800px", "500px");

        }
        function editGroupMember(id) {
            jh.openDialogView("查看群成员","${ctx}/iim/layGroup/member-edit?id="+id,"800px", "500px");

        }
        function outGroup(id) {
            jh.confirm("确认退出该群吗？",function () {
                jh.get('${ctx}/iim/layGroup/logout?user.id=${fns:getUser().id}&group.id='+id, function (data) {
                    if(data.success){
                        jh.success("退出成功!");
                        refreshGroups();
                    }
        ...
        function refreshFriends() {
            jh.get("${ctx}/iim/contact/myFriends", function (data) {
        ...
        function addFriend(){
            jh.openUserSelectDialog(true,function (ids) {
                jh.get("${ctx}/iim/contact/addFriend?ids="+ids, function (result) {
                    if(result.success){
                        refreshFriends()
                        jh.success(result.msg);
                    }
                })
            });
        };
        function delFriend(id){
            jh.get("${ctx}/iim/contact/delFriend?id="+id,function (data) {
				if(data.success){
                    refreshFriends()
				    jh.success(data.msg);
				}
