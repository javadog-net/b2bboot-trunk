window.onload = function () {
	...
	$.ajax({
		type: "POST",
		url: "../../api/web/app/draw/startdraw",
		dataType: 'json',
		async: false,
		data: {
			actvid: activeid,
	...
	function getnum() {
		...
		// debugger;
		for (var i = 0; i < zhongjiang; i++) {
			// 内定人数等于中奖人数
			if (tiancong.length == zhongjiang) {
				for (var h = 0; h < tiancong.length; h++) {
					arr.push(tiancong[h]);
				}
				break;
				// 内定人数大于中奖人数
			} else if (tiancong.length > zhongjiang) {
				for (var h = 0; h < zhongjiang; h++) {
					arr.push(tiancong[h]);
				}
				break;
			} else {
				// 内定人数小中奖人数
				for (var z = 0; z < tiancong.length; z++) {
					if (i > 1) {
						break;
					}
					arr.push(tiancong[z]);
				...
				var ddd = tiancong.length;
				var changdu = zhongjiang - ddd;
				for (var b = 0; b < changdu; b++) {
					arr.push(tiancongnew[b]);
				}
				break;
			}
		}
	...
	//内定人员
	// function huoquneiding(chouren){
	// 	chouren=chouren-0;
	// 	var shengyu=0;
	// 	//内定人数少于抽奖人数
	// 	if(chouren>tiancong.length){
	// 		shengyu=chouren-tiancong.length;
	// 		return suijichouq(tiancong,shengyu)
	// 	}else{
	// 		tiancong.splice(0,chouren)
	// 		console.log(tiancong)
	// 	}
	// 	//TODO 返回一个中奖者
	// 	return tiancong
	// }
	//随机抽取
	// function suijichouq(neidingjihe,num){
	// 	if(neidingjihe){
	// 		console.log(neidingjihe)
	// 		lucky=lucky.concat(neidingjihe)
	// 	}
	// 	while(num>0){
	// 		var index = Math.floor(Math.random()*tiancongnew.length)
	// 		if(tiancongnew[index]==undefined){
	// 			debugger
	// 		}
	// 		console.log("index---------------"+index)
	// 		console.log(tiancongnew[index])
	// 		lucky.push(tiancongnew[index])
	// 		tiancongnew.splice(index,1);
	// 		num--;
	// 	}
	// }
