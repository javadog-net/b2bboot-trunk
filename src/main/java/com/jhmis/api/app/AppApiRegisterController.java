...
public class AppApiRegisterController extends BaseController {
	/**
	 * 获取短信验证码
	 * @param mobile
	 * @return
	 */
	@ApiOperation(notes = "getMobileValidateCodeByApp", httpMethod = "POST", value = "获取短信验证码", consumes = "application/x-www-form-urlencoded")
	@ApiImplicitParams({
			@ApiImplicitParam(name = "mobile", value = "手机", required = true, paramType = "form", dataType = "String")
	})
	@PostMapping("/getMobileValidateCodeByApp")
     ...
     * @return
     */
    @ApiOperation(notes = "checkMobileCodeByApp", httpMethod = "POST", value = "获取短信验证码", consumes = "application/x-www-form-urlencoded")
    @ApiImplicitParams({
    ...
    @PostMapping(value="/checkMobileCodeByApp")
    ...
    @ApiOperation(notes = "userIsRegisteredByApp", httpMethod = "POST", value = "判断用户是否已经注册", consumes = "application/x-www-form-urlencoded")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "mobilephone", value = "手机", required = true, paramType = "form", dataType = "String"),
    })
    @PostMapping(value="/userIsRegisteredByApp")
    ...
    @ApiOperation(notes = "registerByApp", httpMethod = "POST", value = "注册", consumes = "application/x-www-form-urlencoded")
    @ApiImplicitParams({
    ...
    public AjaxJson register(String userName, String mobile, String email, String password, String code,
    		              ...
    		              String companyName,String apptab,String depart,String licenceUrl){
    ...
    //app注册二版 简化注册
    @ApiOperation(notes = "registerByAppTwo", httpMethod = "POST", value = "注册", consumes = "application/x-www-form-urlencoded")
    @ApiImplicitParams({
    		@ApiImplicitParam(name = "apptab", value = "apptab", required = true, paramType = "form", dataType = "String"),
            @ApiImplicitParam(name = "userName", value = "用户名", required = true, paramType = "form", dataType = "String"),
            @ApiImplicitParam(name = "mobile", value = "手机", required = true, paramType = "form", dataType = "String"),
    ...
    @PostMapping("/registerByAppTwo")
    public AjaxJson registerByApp(String userName, String mobile, String email, String password,
    		                      String code,String apptab){
    ...
    //PC简化注册
    @ApiOperation(notes = "registerByPC", httpMethod = "POST", value = "注册", consumes = "application/x-www-form-urlencoded")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "mobile", value = "手机", required = true, paramType = "form", dataType = "String"),
            @ApiImplicitParam(name = "code", value = "短信验证码", required = true, paramType = "form", dataType = "String")
    })
    @PostMapping("/registerByPC")
    public AjaxJson registerByPC(String mobile,String code){
    	if(StringUtils.isBlank(mobile)){
            return AjaxJson.fail("手机不能为空");
        }
        if(StringUtils.isNotBlank(mobile)){
        ...
        //校验用户是否已经注册了
        ExecuteResult<String> usernameResult = UserApi.userIsRegistered(mobile);
        if(!usernameResult.isSuccess()){
            return AjaxJson.fail("该账号已经注册了，请直接登录");
        }
        ExecuteResult<HaierUserDTO> result= UserApi.userRegister(mobile, mobile, "", "haierb2b", code);
        ...
        //PurchaserAccount account = this.getByName(userName);
            ...
            purchaser.setCreateDate(new Date());
            purchaser.setAuditState(Global.AUDIT_STATE_UNSUBMIT);//待完善状态
            purchaser.setChannel("企业购");
            purchaserService.save(purchaser);
            account.setPurchaserId(purchaser.getId());
            account.setLoginName(mobile);
            account.setCreateDate(new Date());
            account.setRealName(mobile);
            account.setPasswd("haierb2b");
            account.setMobile(mobile);
            account.setIsAdmin("1");
            account.setIsClosed("0");
            account.setHaierUserId(String.valueOf(haierUser.getUserId()));
