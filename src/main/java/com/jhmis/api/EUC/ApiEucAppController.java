 ...
 * @Description: EUC与大客户app相关接口提供
 */
@Api(value = "ApiEucAppController", description = "企业购给大客户app提供EUC相关接口")
@RestController
@RequestMapping("/api/eucapp")
public class ApiEucAppController extends BaseController {
    ...
    public AjaxJson garbList(String orgId, HttpServletRequest request, HttpServletResponse response) {
        Page<EucMsg> pageEucMsg = null;
        try {
            pageEucMsg = eucMsgService.garbList(orgId,request,response);
        } catch (Exception e) {
    ...
    public AjaxJson dealerRobtrade(String eucMsgId,String dealerCode,String dealerName) {
        ...
        try {
            if(lockMsgId) {
                eucMsgService.dealerRobtrade(eucMsgId, dealerCode, dealerName);
            }
        } catch (Exception e) {
        ...
        return AjaxJson.ok(EucMsgCode.DEALERROBTRADE_SUCCESS.getValue());
    }
    ...
    public AjaxJson dealerUnderTake(String undertake,String dealerCode, String dealerName,String orderId) {
        List<EucMsg> listEucMsg = null;
        try {
            eucMsgService.dealerUnderTake(undertake, dealerCode,dealerName, orderId);
        } catch (Exception e) {
    ...
    public AjaxJson eucMsgAbandon(String dealerCode,String orderId,String abandonType,String abandonRemark,String abandonWrite) {
        try {
            eucMsgService.eucMsgAbandon(dealerCode, orderId, abandonType,abandonRemark, abandonWrite);
        } catch (Exception e) {
    ...
    public AjaxJson euc_list(EucMsgOrder eucMsgOrder, HttpServletRequest request, HttpServletResponse response) {
        ...
        //eucMsg.setContractorCode("8700104189");
        //是否是top用户
        if(com.jhmis.common.utils.StringUtils.isNotBlank(eucMsgOrder.getIfTopCustomer())){
        ...
        Page<EucMsgOrder> page = null;
        try{
            page = eucMsgOrderService.findPage(p, eucMsgOrder);
        }catch (Exception e){
            e.getMessage();
        }
    ...
    public AjaxJson dealerOrderList(String dealerCode,String flag,String type,String orgId
        ...
        try {
            pageEucMsg = eucMsgService.dealerOrderList(dealerCode, flag, type, request, response);
        } catch (Exception e) {
    ...
    public static boolean isValid(String s) {
        ...
        for(int i=0; i<kh.length; i++){
            if(stack.size()==0){
                ...
                if(stack.peek().equals(trans(kh[i]))){
                    stack.pop();
                }else{
                    stack.push(kh[i]);
                }
