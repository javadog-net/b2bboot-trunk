...
@Api(value = "ApiDealerMsgController", description = "供应商消息管理")
@RestController
@RequestMapping("/api/dealer/msg")
public class ApiDealerMsgController {
    @Autowired
    ...
    @RequestMapping("/list")
    public AjaxJson list(DealerMsg msg, HttpServletRequest request, HttpServletResponse response) {
        ...
        msg.setDealerId(currentAccount.getDealerId());
        msg.setAccountId(currentAccount.getId());
        Page<DealerMsg> page = dealerMsgService.findPage(new Page<DealerMsg>(request, response), msg);
    ...
    public AjaxJson unReadCount(){
        ...
        //分别取得各种类型的消息数量，以及总数量
        int sysMsgCount = dealerMsgService.unReadCount(Global.MESSAGE_TYPE_SYS, currentAccount.getId());
        int orderMsgCount = dealerMsgService.unReadCount(Global.MESSAGE_TYPE_ORDERS, currentAccount.getId());
        int biddingMsgCount = dealerMsgService.unReadCount(Global.MESSAGE_TYPE_BIDDINGS, currentAccount.getId());
    ...
    public AjaxJson read(String id) {
        ...
        DealerMsg msg = dealerMsgService.get(id);
        if (msg == null) {
            return AjaxJson.fail("无此消息");
        }
        dealerMsgService.readMsg(id,currentAccount.getId());
        return AjaxJson.ok();
    }
    ...
    public AjaxJson readAll() {
        ...
        dealerMsgService.readAllMsg(currentAccount.getId());
        return AjaxJson.ok();
    }
    ...
    public AjaxJson del(String id) {
        ...
        if(Global.MESSAGE_TYPE_SYS.equals(msg.getType())){
            return AjaxJson.fail("系统消息不能删除");
        }
        dealerMsgService.delete(msg);
        return AjaxJson.ok();
    }
