...
public class DirectTimeTask {
   ...
   //生产时间，暂时注解
        ...
        List<Map<String, Object>> resObj = (List<Map<String, Object>>) jdbcTemplate1.execute(new PreparedStatementCreator() {
            ...
            public List<Map<String, Object>> doInPreparedStatement(PreparedStatement ps) throws SQLException, DataAccessException {
                ...
                try{
                    rs = ps.getResultSet();
                    //先删后插
                    mdmCustomersSourceService.executeDeleteSql("DELETE FROM mdm_customers_source");
                    while(rs.next()){
    ...
    //0 0 1 * * ? 每天1点执行一次---mdm四方关系
    //生产时间，暂时注解  
    //@Scheduled(cron = "*/10 * * * * ?")
    @Scheduled(cron = "0 0 1 * * ?")
    public List<Map<String, Object>> getMdmCustomersPartner(){
        ...
        List<Map<String, Object>> resObj = (List<Map<String, Object>>) jdbcTemplate1.execute(new PreparedStatementCreator() {
            ...
            public List<Map<String, Object>> doInPreparedStatement(PreparedStatement ps) throws SQLException, DataAccessException {
                ...
                try {
                    rs = ps.getResultSet();
                    //先删后插
                    mdmCustomersPartnerService.executeDeleteSql("DELETE FROM mdm_customers_partner");
                    while(rs.next()){
