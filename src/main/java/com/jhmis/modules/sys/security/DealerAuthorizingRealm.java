 ...
 * 供应商认证
 */
public class DealerAuthorizingRealm extends AuthorizingRealm {
    @Override
    public String getName() {
        return Global.USER_TYPE_DEALER;
    }
    /**
     * 必须重写此方法，不然Shiro会报错
     */
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
        ...
        //同时下面构造的principal里面放入dealer的相关数据
        //提交给AuthenticationInfo进行密码校验
        Principal principal = new Principal(userId,account.getLoginName(),account.getRealName(),false,Global.USER_TYPE_DEALER,token);
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        if (account != null) {
            ...
            info.addRole("dealer");
            // 添加用户角色信息
            /*for (DealerRole role : account.getRoleList()){
