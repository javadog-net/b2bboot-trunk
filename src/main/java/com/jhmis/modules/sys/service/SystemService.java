...
import com.jhmis.core.security.Digests;
...
public class SystemService extends BaseService {
	...
	@Autowired
	private GmInfoMapper gmInfoMapper;
	@Autowired
	...
	public void saveUser(User user) {
		...
		if (StringUtils.isNotBlank(user.getGmId())) {
			GmInfo g = gmInfoMapper.findUniqueByProperty("branch_code",user.getGmId());
			user.setGmName(g.getGmName());
		}
		if (StringUtils.isBlank(user.getId())) {
			user.preInsert();
			if(user.getIsSysadmin().contains(",")){
			...
			// 更新用户数据
			user.preUpdate();
			if (StringUtils.isNotBlank(user.getIsSysadmin()) && user.getIsSysadmin().equals("1")&&
					!user.getIsSysadmin().contains("0")
					) {
				user.setGmId("");
				user.setGmName("");
			}
