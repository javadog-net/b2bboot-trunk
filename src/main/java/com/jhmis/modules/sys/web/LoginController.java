...
import com.jhmis.modules.iim.service.MailBoxService;
import com.jhmis.modules.sys.security.FormAuthenticationFilter;
...
public class LoginController extends BaseController{
	...
	public String index(HttpServletRequest request, HttpServletResponse response) {
		...
		if(UserUtils.getMenuList().size() == 0){
            ...
            // 默认风格
            String indexStyle ;
            if(JhUtils.checkAgentIsMobile(request.getHeader("User-Agent"))){//如果是移动，默认使用左侧菜单，显示效果较好
                indexStyle = "ani";
            }else{//如果是pc端，默认使用配置项，jp横向菜单，ani左侧菜单
                indexStyle = Global.getConfig("defaultTheme","ani");
            }
            ...
            if (StringUtils.isNotEmpty(indexStyle)
                    && indexStyle.equalsIgnoreCase("adv")) {
                return "modules/sys/login/sysIndex-adv";
            }
            return "modules/sys/login/sysIndex";
		}
