 ...
 */
package com.jhmis.modules.iim.web;
...
import com.jhmis.modules.iim.entity.*;
import com.jhmis.modules.iim.service.LayGroupService;
...
import com.jhmis.modules.sys.service.OfficeService;
...
public class ContactController extends BaseController {
	...
	public HashMap uploadImage(HttpServletRequest request, HttpServletResponse response)
			throws IllegalStateException, IOException, NoSuchAlgorithmException {
		LayFileJsonData data = new LayFileJsonData();
		AjaxJson ret = uploadService.uploadImages(request,null,Global.USER_TYPE_SYS,UserUtils.getPrincipal().getId(),false,0,0);
		List<AttInfo> attInfos = (List<AttInfo>) ret.getResult();
		if(attInfos!=null && attInfos.size()>0){
			data.setName(attInfos.get(0).getAttName());
			data.setSrc(Global.getUploadHost() + attInfos.get(0).getAttUrl());
		}
	...
	public HashMap uploadFile(HttpServletRequest request, HttpServletResponse response, MultipartFile file)
			throws IllegalStateException, IOException, NoSuchAlgorithmException {
		LayFileJsonData data = new LayFileJsonData();
		AjaxJson ret = uploadService.uploadFiles(request,null,Global.USER_TYPE_SYS,UserUtils.getPrincipal().getId());
		List<AttInfo> attInfos = (List<AttInfo>) ret.getResult();
		if(attInfos!=null && attInfos.size()>0){
			data.setName(attInfos.get(0).getAttName());
			data.setSrc(Global.getUploadHost() + attInfos.get(0).getAttUrl());
...
/*
	...
	public HashMap uploadImage(HttpServletRequest request, HttpServletResponse response, MultipartFile file)
		...
		if (!file.isEmpty()) {
			...
			String newFileName = FileUtils.genFileName(file.getOriginalFilename());
			...
			//考虑数据库里面应该存储虚拟路径，不应该要contextPath,这样修改部署路径后无法移植
