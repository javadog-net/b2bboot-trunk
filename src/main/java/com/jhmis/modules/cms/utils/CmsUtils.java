...
public class CmsUtils {
	...
	public static final String CMS_CONFIG_CACHE = "cmsConfigCache";
	private static final String CMS_CACHE = "cmsCache";
	private static ServletContext context = SpringContextHolder.getBean(ServletContext.class);
	private static CmsConfigService cmsConfigService = SpringContextHolder.getBean("cmsConfigService");
	...
	public static  CmsConfig getCmsConfig(){
		//先从缓存中查找，如果存在直接取缓存中的配置信息
		CmsConfig cmsConfig = (CmsConfig) CmsUtils.getCache(CMS_CONFIG_CACHE);
		if(cmsConfig != null){
			return cmsConfig;
		}else{
			//如果缓存中不存在，从数据库中查询，然后放入缓存
			cmsConfig = new CmsConfig();
			List<CmsConfig> cmsConfigList = cmsConfigService.findList(cmsConfig);
			if(cmsConfigList.size()>0){
				cmsConfig = cmsConfigList.get(0);
			}
			CmsUtils.putCache(CMS_CONFIG_CACHE,cmsConfigList.get(0));
		}
		return cmsConfig;
	}
	...
	public static String  getTemplateStyle(){
		CmsConfig cmsConfig = getCmsConfig();
		if(cmsConfig != null && StringUtils.isNotEmpty(cmsConfig.getTempletStyle())){
			return cmsConfig.getTempletStyle();
		}else
			return null;

	}
	public static String  getTemplateWholePath(String templatePath){
		CmsConfig cmsConfig = getCmsConfig();
		if(cmsConfig != null && StringUtils.isNotEmpty(cmsConfig.getTempletStyle())){
			return templatePath+cmsConfig.getTempletStyle();
		}else
			return templatePath;
...
//			category.setParent(new Category("0"));
