 ...
 * 下一篇文章标签
 * @author lydia
 * @date 2019/10/09 11:01
 */
@FreemarkerComponent("infoNextList")
public class InfoNextDirective extends BaseDirective {
    @Autowired
        ...
        if (body != null) {
            ...
            if (loopVars != null && loopVars.length > 0) {
                ...
                if (StringUtils.isNotEmpty(infoId)) {
                    ...
                    if ("1".equals(hot)) {
                        if (currInfo != null) {
                            info.setInfoEndHits(currInfo.getHits());
                            info.setInfoendtimeNoeq(currInfo.getCreateDate());
                            info.setOrderBySql("a.hits desc,a.create_date desc");
                        }
                    } else {
                        if ("1".equals(order)) {
                            //固顶有效并降序,发布时间降序(默认)
                            if (currInfo != null) {
                                info.setInfoendtimeNoeq(currInfo.getCreateDate());
                                info.setOrderBySql(" a.is_top desc,a.create_date desc");
                            }
                        } else if ("2".equals(order)) {
                            //固顶有效并降序,发布时间升序
                            if (currInfo != null) {
                                info.setInfostarttimeNoeq(currInfo.getCreateDate());
                                info.setOrderBySql("a.is_top ,a.create_date");
                            }
