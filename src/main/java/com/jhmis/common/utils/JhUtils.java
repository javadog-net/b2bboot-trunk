...
public class JhUtils {
	...
	private static Logger log = LoggerFactory.getLogger(JhUtils.class);
	...
	public static String getFixedPersonIDCode(String personIDCode) {
		if (personIDCode == null)
			throw new IllegalArgumentException("输入的身份证号无效，请检查");
	...
	public static String fixPersonIDCodeWithCheck(String personIDCode) {
		if (personIDCode == null || personIDCode.trim().length() != 15)
			throw new IllegalArgumentException("输入的身份证号不足15位，请检查");
	...
	public static Timestamp getBirthdayFromPersonIDCode(String identity) {
		...
		try {
			...
			throw new IllegalArgumentException("不是有效的身份证号，请检查", e);
		}
	}
	...
	public static String getWeekDayByDate(String strdate) {
		...
		try {
			...
			throw new IllegalArgumentException("日期类型转换出错", e);
		}
	...
	public static String getPathFromClass(Class cls) {
		...
		if (url != null) {
			...
			try {
				...
				throw new IllegalArgumentException("获取class文件所在绝对路径出错", e);
			}
		}
		return path;
	}
	...
	public static String readStringFromFile(File file, String encoding) {
		...
		try {
			...
			throw new RuntimeException("从文件读取字符串失败", e);
		} finally {
			if (reader != null)
				try {
					reader.close();
